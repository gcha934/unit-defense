...
function SpawnCreeps()
  ...
  Position = pos1, --Optional.  Only used when targeting the ground
  Queue =true, }
  
 end
--[[function pos1check()
   if Entities:FindByNameNearest("sheep", pos1:GetAbsOrigin(),  16) ~=nil then 
     local path = {
     UnitIndex = Entities:FindByNameNearest("sheep", pos1:GetAbsOrigin(),  16):getEntityIndex(), 
      OrderType = DOTA_UNIT_ORDER_MOVE_TO_POSITION,
      Position = pos2, --Optional.  Only used when targeting the ground
      Queue =true,
        }
     end
  end
